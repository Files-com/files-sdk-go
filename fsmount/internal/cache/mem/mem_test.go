...
// TestNewMemoryCache verifies that a MemoryCache can be created with default settings
func TestNewMemoryCache(t *testing.T) {
	...
	if stats.CapacityBytes != mem.DefaultCapacity {
		t.Errorf("Expected capacity %d, got %d", mem.DefaultCapacity, stats.CapacityBytes)
	}

	if stats.MaxFileCount != mem.DefaultMaxFileCount {
		t.Errorf("Expected max file count %d, got %d", mem.DefaultMaxFileCount, stats.MaxFileCount)
	}
}

// TestMemoryCacheWriteAndRead verifies basic write and read operations
func TestMemoryCacheWriteAndRead(t *testing.T) {
	cache, err := mem.NewMemoryCache()
	if err != nil {
		t.Fatalf("NewMemoryCache failed: %v", err)
	}
...
// TestMemoryCacheWriteAtOffset verifies writing at a non-zero offset
func TestMemoryCacheWriteAtOffset(t *testing.T) {
	cache, err := mem.NewMemoryCache()
	if err != nil {
		t.Fatalf("NewMemoryCache failed: %v", err)
	}
...
// TestMemoryCacheReadMiss verifies that reading a non-existent file returns 0 bytes
func TestMemoryCacheReadMiss(t *testing.T) {
	cache, err := mem.NewMemoryCache()
	if err != nil {
		t.Fatalf("NewMemoryCache failed: %v", err)
	}
...
// TestMemoryCacheDelete verifies that deleting a file removes it from the cache
...
// TestMemoryCachePinUnpin verifies that pinned files are not evicted
func TestMemoryCachePinUnpin(t *testing.T) {
	// Create cache with very small capacity to force evictions
	cache, err := mem.NewMemoryCache(mem.WithCapacityBytes(mem.MinCapacity))
	if err != nil {
		t.Fatalf("NewMemoryCache failed: %v", err)
	}
	...
	size := (mem.MinCapacity / 2) + 100 // Each file is larger than half the capacity
	data := make([]byte, size)
...
// TestMemoryCacheStats verifies that statistics are updated correctly
...
// TestMemoryCacheMaintenance verifies that maintenance runs without error
func TestMemoryCacheMaintenance(t *testing.T) {
	// Create cache with short maintenance interval
	cache, err := mem.NewMemoryCache(
		mem.WithMaintenanceInterval(100 * time.Millisecond),
	)
	if err != nil {
		t.Fatalf("NewMemoryCache failed: %v", err)
	}
...
// TestMemoryCacheMultipleFiles verifies that multiple files can coexist in the cache
func TestMemoryCacheMultipleFiles(t *testing.T) {
	cache, err := mem.NewMemoryCache()
	if err != nil {
		t.Fatalf("NewMemoryCache failed: %v", err)
	}
...
// TestMemoryCacheCapacityEviction verifies that files are evicted when capacity is exceeded
func TestMemoryCacheCapacityEviction(t *testing.T) {
	...
	buf := make([]byte, 10)
	n1, err := cache.Read(path1, buf, 0)
	if err != nil {
		t.Fatalf("Read file1 failed: %v", err)
	}
	n2, err := cache.Read(path2, buf, 0)
	if err != nil {
		t.Fatalf("Read file2 failed: %v", err)
	}

	if n1 != 0 && n2 != 0 {
		t.Error("Expected at least one of file1 or file2 to be evicted due to capacity")
	}
}

// TestMemoryCacheUnpinEviction verifies that an unpinned file is evicted when the cache is over capacity,
...
func TestMemoryCacheUnpinEviction(t *testing.T) {
	cache, err := mem.NewMemoryCache(mem.WithCapacityBytes(mem.MinCapacity))
	if err != nil {
		t.Fatalf("NewMemoryCache failed: %v", err)
	}

	dataSize := (mem.MinCapacity / 2) + 100 // just over more than half capacity
	...
	if statsBefore.SizeBytes.Load() <= mem.MinCapacity {
		t.Logf("Cache size before unpin: %d (expected > %d)", statsBefore.SizeBytes.Load(), mem.MinCapacity)
	}
