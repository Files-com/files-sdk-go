...
## [2.0.110-beta] - 2022/11/03
### Fix
- Remote paths for `file.Uploader`/`file.Downloader` that start with a slash are now normalized, fixing a possible panic.
...
## [2.0.87-beta] - 2022/06/29
### Fix
- Model fields of `time.Time` are changed to `*time.Time`. This gets around a zero value being send to the API.
